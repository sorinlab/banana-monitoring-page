        ...
        <div id="header">
            <h2> SORIN LAB RAID INFO </h2>
             <ul id="mainNav">
                 ...
                 <li><a 
href="http://folding.cnsm.csulb.edu/wiki/index.php/Machine_Monitoring_Page">Monitor-Info</a></li>
            </ul>
        </div>
        <!-- Critical elements alert -->
        <span id="criticalMessage" style="display:none;"> </span>
        <div id="servers">
            <h2> SERVERS </h2>
            <?php
            //Array of file names that store server info
            // To add a new server add the new file names here
            $filenames = array('banana-DataRaid.txt','banana-OSRaid.txt','storage1-Data.txt','storage1-OS.txt',
                // 'storage2-DataRaid.txt','storage2-OSRaid.txt',
                'entropy1.txt', 'entropy1-vdisk.txt', 'folding1.txt','folding2.txt');
		...
		$MNAME = "NOTBANANA";
                ...
                /* OLD banana that used a hardware raid controller along with software raid
                this tower was retired and replaced by what used to be sorin9 (Khai Nguyen)
                if(strpos($name,"bananahw")!==FALSE){
                    echo "<h3 class=\"machineName\">banana Hardware RAID</h3>";
                }
                if(strpos($name,"bananasw")!==FALSE){
                    echo "<h3 class=\"machineName\">banana Software RAID</h3>";
                    //Smart support enabled for banana-sw RAID hence add the file and disk names
                    $smartfile2 = "smartctl/banana-sda.txt"; $smartdisk2 = "/dev/sda";
                    $smartfile1 = "smartctl/banana-sdb.txt"; $smartdisk1 = "/dev/sdb";
                    $smartEnabledDisks = 2;
                }*/
		    ...
		    $MNAME = "entropy1";
                ...
                }/* Perutz is not longer online
                if (strpos($name,"perutz") !== FALSE) {
                    echo "<h3 class=\"machineName\">perutz</h3>";
                }*/
		    ...
		    $smartfile1 = "smartctl/folding2-sda.txt"; $smartdisk1 = "/dev/sda";
                ...
                // storage2 was made the new banana on around Feb 20, 2015 (Khai Nguyen)
                /* if(strpos($name,"storage2-OS")!==FALSE) {
                            echo "<h3 class=\"machineName\">storage2 OS RAID</h3>";
                             $smartfile1 = "smartctl/storage2-sdc.txt"; $smartdisk1 = "/dev/sdc";
                             $smartfile2 = "smartctl/storage2-sdd.txt"; $smartdisk2 = "/dev/sdd";
                             $smartEnabledDisks = 2;
                    }
                    if(strpos($name,"storage2-Data")!==FALSE) {
                            echo "<h3 class=\"machineName\">storage2 Data RAID</h3>";
                             $smartfile1 = "smartctl/storage2-sda.txt"; $smartdisk1 = "/dev/sda";
                             $smartfile2 = "smartctl/storage2-sdb.txt"; $smartdisk2 = "/dev/sdb";
                             $smartEnabledDisks = 2;
                    } */
                ...
                if (file_exists($name)) {
                    ...
                    if ($name === 'perutz.txt' || $name === 'folding1-raidstat.txt'
                        || $name === 'folding2-raidstat.txt') {
			...
			// Note: we prevent output for entropy1's Virtual Disk. It uses omreport.
                        ...
                        //Code specific to banana hardware RAID
                        /* if(strpos($line,"ioc0")!==FALSE){
                           //check if the ioc0 is ONLINE or ENABLED else display an error
                        ...
                        if (strpos($line,"Array Size")!==FALSE ||
                            strpos($line,"Active Devices")!==FALSE ||
                            strpos($line,"Working Devices")!==FALSE ||
                            strpos($line,"Controller")!==FALSE ||
                            strpos($line,"Disk Space")!==FALSE) {
                                echo "<li>".$line."</li>";
                        }
...
Attribute is said to have failed.
                        ...
                        */
                        ...
                        if (strpos($line,"Short offline")!==FALSE &&
                            strpos($line,"Completed without error")!==FALSE &&
                            $shortOffline < 4) {
                            $shortOffline++;
                        ...
                        if (strpos($line,"Extended offline")!==FALSE &&
                            strpos($line,"Completed without error")!==FALSE &&
                            $extendedOffline < 4) {
                            $extendedOffline++;
                            if (preg_match_all('!\d+!', $line, $matches)) {
	...
	<div id="desktops">
            ...
            for($i=1;$i<=13;$i++){
             ...
             foreach ($fh as $line){
                 ...
                 if(strpos($line,"Update Time") !==FALSE || strpos($line,"Generated On") !==FALSE){
                   //Extract the update time from the line
                   if(strpos($line,"Update Time") !==FALSE){$date_string = preg_replace('/Update Time : /', 
'',$line);}
                   if(strpos($line,"Generated On") !==FALSE){$date_string = preg_replace('/Generated On : /', 
'',$line);}
                   // Convert the string to time in seconds
                      $updatetime = strtotime($date_string);
                      //Caliculate the present time
                  ...
                  if(strpos($line,"State :") !==FALSE){
                     ...
                     if(strpos($line,"clean")!==FALSE ||
                         ...
                         //Further check to see if the line contains inactive, degraded, idle
                         if(strpos($line,"inactive")!==FALSE ||
                          strpos($line,"degraded")!==FALSE || strpos($line,"idle")!==FALSE){
                             echo "<li class=\"error\">".$line."</li>";
                         }else{
                         echo "<li class=\"active\">".$line."</li>";
                     }}
                  ...
                  if(strpos($line,"Active Devices")!==FALSE ||
                         strpos($line,"Working Devices")!==FALSE){
                     echo "<li>".$line."</li>";
                 }
                 if(strpos($line,"Failed Devices")!==FALSE){
		...
		//Edited by Xavier Martinez on 11/13/16 # sorin11 no longer has 3HDD #
		//if($j==3 and $i==11){
	...
	if($j<=2){
                ...
                foreach ($fh as $line){
                  ...
                  //Now compute the average life time of the disk
                  //By considering latest 4 short offline & 4 extended offline lifetime values and computing an average
                  if(strpos($line,"Short offline")!==FALSE && strpos($line,"Completed without error")!==FALSE 
&& $shortOffline < 4){
                     $shortOffline++;
                      if (preg_match_all('!\d+!', $line, $matches)) {
                            if(isset($matches[0][2])){
                  ...
                  if(strpos($line,"Extended offline")!==FALSE && strpos($line,"Completed without 
error")!==FALSE && $extendedOffline < 4){
                        $extendedOffline++;
                      if (preg_match_all('!\d+!', $line, $matches)) {
                            if(isset($matches[0][2])){
                            if($matches[0][2] > 1000){
	...
	if($j==3 && $i==12){
		 //Check if the file exists
	      ...
	      }//End of if($j==3 && ($i==11 || $i==12))
             }// Inner For loop
            }//End of sorin13 if statement
            echo "</ul>"; //End of machine info
            echo "</div>"; // End of Div
